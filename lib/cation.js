...
var cation = module.exports = function(parameters, services) {
   ...
   * You can get a value by using JSON notation
   * to access JSON parameters. Ie: getParameter('app.locales.en.i18n').
   * @param  {string} name Name of parameter
   * @return {[type]} Parameter value
   */
  cation.getParameter = function(name) {
    var multi_parts = name.split('.'),
        values  = parameter_bag;

    for (var i=0;i<multi_parts.length; i++) {
      if (typeof values == 'undefined') {
        break;
      }

      values = values[multi_parts[i]];
    }
  ...
  var loadService = function(service_alias, onSuccess, onFailure) {
    ...
    if (!cation.hasLoaded(service_alias)) { //prevent loading already loaded service
      srv_require = replaceParameters(srv_require); //parameters %param% will be replaced with their values
      ...
      if (srv_builder instanceof Array) { // ensure that wants to build the require
        if (srv_builder.length) { // if it's defined
